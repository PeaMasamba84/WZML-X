            ...
            if msg.startswith('magnet'):
                ...
                for check in mag:
                    if check.startswith('tr='):
                        ...
                        amper = True
                self.source_msg = f"<b>Name:<blockquote></b> <i>{name}</i>\n<b>Magnet Hash:</b> <code>{hashh}</code>\n<b>Total Trackers:</b> {tracCount} \n<b>Share:</b> <a href='https://t.me/share/url?url={quote(msg)}'>Share To Telegram</a><blockquote>"
            else:
        ...
        if self.isLeech:
            ...
            if config_dict['MIRROR_LOG_ID'] and not self.excep_chat:
                ...
                if self.linkslogmsg:
                    ...
                    await editMessage(self.linkslogmsg, (msg + BotTheme('LINKS_SOURCE', On=dispTime, Source=self.source_msg) + BotTheme('LL_MSG') + f"\n\n<a href='{log_msg.link}'>{escape(name)}</a>\n"), _btns.build_menu(1))
