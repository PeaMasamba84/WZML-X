...
def BotTheme(var_name, **format_vars):
    ...
    if theme_ in AVL_THEMES:
        text = getattr(AVL_THEMES[theme_].PEAMASAMBAStyle(), var_name, None)
        if text is None:
        ...
        text = getattr(rantheme.PEAMASAMBAStyle(), var_name, None)
        
    if text is None:
        text = getattr(wzml_minimal.PEAMASAMBAStyle(), var_name)
